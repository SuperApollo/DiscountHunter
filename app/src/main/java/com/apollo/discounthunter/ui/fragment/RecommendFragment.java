...
public class RecommendFragment extends BaseFragment {
    ...
    XListView mXlvRecommend;
    List<Model> mRecommendModels = new ArrayList<>();
    ...
    private Handler mHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case STOP_REFRESH:
                    mXlvRecommend.stopRefresh();
                    break;
                case STOP_LOADMORE:
                    mXlvRecommend.stopLoadMore();
                    break;
                case TOP_BTN_GONE:
    ...
    protected void init() {
        ...
        mXlvRecommend.setVerticalScrollBarEnabled(false);
        mXlvRecommend.setPullLoadEnable(true);
        mXlvRecommend.setPullRefreshEnable(true);
        mXlvRecommend.setXListViewListener(new XListView.IXListViewListener() {
            @Override
        ...
        mXlvRecommend.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override
            public void onItemClick(AdapterView<?> adapterView, View view, int i, long l) {
                Model homeModel = mRecommendModels.get(i - 1);
        ...
        mXlvRecommend.setOnScrollListener(new XListView.OnXScrollListener() {
            @Override
            ...
            public void onScrollStateChanged(AbsListView absListView, int i) {
                firstVisiblePosition = mXlvRecommend.getFirstVisiblePosition();
                if (i == 0) {//停止滑动
        ...
        mAdapter = new HomeListAdapter(mContext, mRecommendModels);
        mXlvRecommend.setAdapter(mAdapter);
        if (firstEnter) {
            requestData();
            firstEnter = false;
        }
        ...
        mBtnToTop.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                mXlvRecommend.smoothScrollToPosition(1);
            }
        });
        mBtnToBottom.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                if (mRecommendModels != null && mRecommendModels.size() > 0)
                    mXlvRecommend.smoothScrollToPosition(mRecommendModels.size() - 1);
            }
        });
    }
    ...
    public void setUserVisibleHint(boolean isVisibleToUser) {
        ...
        if (mXlvRecommend != null && firstVisiblePosition > 0) {
            mXlvRecommend.setSelection(firstVisiblePosition);
        }
    }
